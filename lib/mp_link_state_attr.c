 ...
 * +-----------------+-------------------------+------------+
 */
const char *const LS_FLAGS_NODE_NLRI[] = {"O", "T", "E", "B", "R", "V"};

/* https://tools.ietf.org/html/draft-gredler-idr-bgp-ls-segment-routing-ext-04#section-2.2.1
 *      ISIS:
 * https://tools.ietf.org/html/draft-ietf-isis-segment-routing-extensions-09#section-2.2.1
 *      OSPF:
 * https://tools.ietf.org/html/draft-ietf-ospf-segment-routing-extensions-10#section-7.1
 ...
 *
 *      ISIS:
 * https://tools.ietf.org/html/draft-ietf-isis-segment-routing-extensions-09#section-3.1
 *      OSPF:
 * https://tools.ietf.org/html/draft-gredler-idr-bgp-ls-segment-routing-ext-04#ref-I-D.ietf-ospf-ospfv3-segment-routing-extensions
 *
 */
const char *const LS_FLAGS_SR_CAP_ISIS[] = {
  "I", // MPLS IPv4 flag; set = router is capable of SR MPLS encaps IPv4 all
       // interfaces
  "V", // MPLS IPv6 flag; set = router is capable of SR MPLS encaps IPv6 all
       // interfaces
  "H"  // SR-IPv6 flag; set = rouer is capable of IPv6 SR header on all
       // interfaces defined in ...
};

/* https://tools.ietf.org/html/draft-gredler-idr-bgp-ls-segment-routing-ext-04#section-2.3.1
 *      ISIS:
 * https://tools.ietf.org/html/draft-ietf-isis-segment-routing-extensions-09#section-2.1.1
 *      OSPF:
 * https://tools.ietf.org/html/draft-ietf-ospf-segment-routing-extensions-10#section-5
 */
const char *const LS_FLAGS_PREFIX_SID_ISIS[] = {
  ...
  "P", // no-PHP flag; set = penultimate hop MUST NOT pop before delivering the
       // packet
  "E", // Explicit-Null flag; set = upstream neighbor must replace SID with
       // Exp-Null label
  "V", // Value flag; set = SID carries a value instead of an index; default
       // unset
...
const char *const LS_FLAGS_PREFIX_SID_OSPF[] = {
  "",   // unused
  "NP", // no-PHP flag; set = penultimate hop MUST NOT pop before delivering the
        // packet
 ...
 */
void libparsebgp_mp_link_state_attr_parse_attr_link_state(
  update_path_attrs *path_attrs, int attr_len, u_char **data)
{
  ...
  int tlv_len;
  uint16_t count = 0;
  path_attrs->attr_value.bgp_ls =
    (bgp_link_state_attrs *)malloc(sizeof(bgp_link_state_attrs));
  while (attr_len > 0) {
    if (count)
      path_attrs->attr_value.bgp_ls = (bgp_link_state_attrs *)realloc(
        path_attrs->attr_value.bgp_ls,
        (count + 1) * sizeof(bgp_link_state_attrs));

    tlv_len = libparsebgp_mp_link_state_attr_parse_attr_link_state_tlv(
      path_attrs, attr_len, data, count);
    attr_len -= tlv_len;
...
/*******************************************************************************/ /**
                                                                                   ...
                                                                                   */
int libparsebgp_mp_link_state_attr_parse_attr_link_state_tlv(
  update_path_attrs *path_attrs, int attr_len, u_char **data, int count)
{
  ...
  if (attr_len < 4) {
    //          LOG_NOTICE("%s: bgp-ls: failed to parse attribute; too
    //          short",peer_addr.c_str());
    return attr_len;
  }

  bgp_link_state_attrs *bgp_ls_attr =
    (bgp_link_state_attrs *)malloc(sizeof(bgp_link_state_attrs));
  ...
  *data += 4;
  ...
  case ATTR_NODE_FLAG: {
    ...
    if (bgp_ls_attr->len != 1) {
      //   //             LOG_INFO("%s: bgp-ls: node flag attribute length is
      //   too long %d should be 1",peer_addr.c_str(), len);
    }
  ...
  case ATTR_NODE_IPV6_ROUTER_ID_LOCAL: // Includes
                                       // ATTR_LINK_IPV6_ROUTER_ID_LOCAL
    if (bgp_ls_attr->len != 16) {
      //               LOG_NOTICE("%s: bgp-ls: failed to parse attribute local
      //               router id IPv6 sub-tlv; too short",peer_addr.c_str());
      break;
    }
...
val_ss.str().data(), val_ss.str().length());*/
  ...
  case ATTR_LINK_ADMIN_GROUP:
    if (bgp_ls_attr->len != 4) {
      //         LOG_NOTICE("%s: bgp-ls: failed to parse attribute link admin
      //         group sub-tlv, size not 4",peer_addr.c_str());
      break;
    } else {
      value_32bit = 0;
...
*)path_attrs->ls_attrs[ATTR_LINK_ADJACENCY_SID].data()));*/
  ...
  case ATTR_LINK_UNRESV_BW: {
    ...
    //     SELF_DEBUG("%s: bgp-ls: parsing link unreserve bw attribute
  ...
  case ATTR_LINK_OPAQUE:
    //     LOG_INFO("%s: bgp-ls: opaque link attribute (len=%d), not yet
    //     implemented", peer_addr.c_str(), len);
    break;
  ...
  case ATTR_PREFIX_ROUTE_TAG: {
    //    SELF_DEBUG("%s: bgp-ls: parsing prefix route tag attribute (len=%d)",
    //    peer_addr.c_str(), len);
  ...
  case ATTR_PREFIX_OSPF_FWD_ADDR:
    // SELF_DEBUG("%s: bgp-ls: parsing prefix OSPF forwarding address
    // attribute", peer_addr.c_str());
    //    LOG_INFO("%s: bgp-ls: prefix OSPF forwarding address attribute, not
    //    yet implemented", peer_addr.c_str());
    break;

  case ATTR_PREFIX_OPAQUE_PREFIX:
    //    LOG_INFO("%s: bgp-ls: opaque prefix attribute (len=%d), not yet
    //    implemented", peer_addr.c_str(), len);
    break;
  ...
  default:
    //        LOG_INFO("%s: bgp-ls: Attribute type=%d len=%d not yet
