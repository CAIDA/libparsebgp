 ...
 * to read; prevent overrun when reading \param [out]     parsed_msg  Reference
 * pointer to where to store the parsed notification message
 *
                                      ...
                                      uint8_t **data, size_t size)
{
  ...
  case NOTIFY_MSG_HDR_ERR: {
    if (parsed_msg->error_subcode == MSG_HDR_BAD_MSG_LEN)
      snprintf(parsed_msg->error_text, sizeof(parsed_msg->error_text),
               "Bad message header length");
    else
      snprintf(parsed_msg->error_text, sizeof(parsed_msg->error_text),
               "Bad message header type");
    break;
  }
  ...
  case NOTIFY_OPEN_MSG_ERR: {
    ...
    default:
      snprintf(parsed_msg->error_text, sizeof(parsed_msg->error_text),
               "Open message error - unknown subcode [%d]",
  ...
  case NOTIFY_UPDATE_MSG_ERR: {
    ...
    case UPDATE_INVALID_NEXT_HOP_ATTR:
      snprintf(parsed_msg->error_text, sizeof(parsed_msg->error_text),
               "Invalid next hop address/attribute");
      break;
    ...
    case UPDATE_OPT_ATTR_ERROR:
      snprintf(parsed_msg->error_text, sizeof(parsed_msg->error_text),
               "Update optional attribute error");
      break;
    ...
    default:
      snprintf(parsed_msg->error_text, sizeof(parsed_msg->error_text),
               "Update message error - unknown subcode [%d]",
               parsed_msg->error_subcode);
      break;
    }
    break;
  ...
  case NOTIFY_CEASE: {
    ...
    default:
      snprintf(parsed_msg->error_text, sizeof(parsed_msg->error_text),
               "Unknown cease code, subcode [%d]", parsed_msg->error_subcode);
      break;
    }
    break;
  }
  ...
  default: {
    sprintf(parsed_msg->error_text, "Unknown notification type [%d]",
