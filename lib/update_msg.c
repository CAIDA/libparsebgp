 ...
 *
 * @details
 *      Will get the NLRI and Withdrawn prefix entries from the data buffer.  As
 * per RFC, this is only for v4.  V6/mpls is via mpbgp attributes (RFC4760)
 *
 * @param [in]   data       Pointer to the start of the prefixes to be parsed
 * @param [in]   len        Length of the data in bytes to be read
 * @param [out]  prefixes   Reference to a list<prefix_tuple> to be updated with
 * entries
 */
static ssize_t
  ...
  for (size_t read_size = 0; read_size < len; read_size++) {
    ...
    if (addr_bytes <= 4) {
      ...
      (*prefixes)[*count_prefix] =
        (update_prefix_tuple *)malloc(sizeof(update_prefix_tuple));
      memcpy((*prefixes)[(*count_prefix)++], prefix_tuple,
             sizeof(update_prefix_tuple));

    } else if (addr_bytes > 4) {
      // LOG_NOTICE("%s: rtr=%s: NRLI v4 address is larger than 4 bytes bytes=%d
      // len=%d",
      //           peer_addr.c_str(), router_addr.c_str(), addr_bytes,
      //           tuple.len);
    }
  }
  free(prefix_tuple);
 ...
 * notification message \param [in]   size           Size of the data available
 * to read; prevent overrun when reading \param [out]  parsed_data    Reference
 * to parsed_update_data; will be updated with all parsed data
 *
                                        ...
                                        int *has_end_of_rib_marker)
{
  ...
  memcpy(&update_msg->wdrawn_route_len, buf_ptr,
         sizeof(update_msg->wdrawn_route_len));
  ...
  if ((size - bytes_check) < update_msg->wdrawn_route_len) {
    // LOG_WARN("%s: rtr=%s: Update message is too short to parse withdrawn
    // data", peer_addr.c_str(), router_addr.c_str());
    return INCOMPLETE_MSG;
  }
  ...
  memcpy(&update_msg->total_path_attr_len, buf_ptr,
         sizeof(update_msg->total_path_attr_len));
  SWAP_BYTES(&update_msg->total_path_attr_len,
             sizeof(update_msg->total_path_attr_len));
  buf_ptr += sizeof(update_msg->total_path_attr_len);
  read_size += sizeof(update_msg->total_path_attr_len);
  bytes_check += sizeof(update_msg->total_path_attr_len);

  // Set the attributes data pointer
  if ((size - bytes_check) < update_msg->total_path_attr_len) {
    // LOG_WARN("%s: rtr=%s: Update message is too short to parse attr data",
    // peer_addr.c_str(), router_addr.c_str());
    return INCOMPLETE_MSG;
  }
  attr_ptr = buf_ptr;
  buf_ptr += update_msg->total_path_attr_len;
  bytes_check += update_msg->total_path_attr_len;
  ...
  if (!update_msg->wdrawn_route_len && (size - bytes_check) <= 0 &&
    ...
    *has_end_of_rib_marker = 1;
    ...
    if (update_msg->wdrawn_route_len > 0) {
      bytes_read = libparsebgp_update_msg_parse_nlri_data_v4(
        withdrawn_ptr, update_msg->wdrawn_route_len, &update_msg->wdrawn_routes,
        &update_msg->count_wdrawn_route);
      if (bytes_read < 0)
        return bytes_read;
    ...
    if (update_msg->total_path_attr_len > 0) {
      bytes_read = libparsebgp_update_msg_parse_attributes(
        &update_msg->path_attributes, attr_ptr, update_msg->total_path_attr_len,
        has_end_of_rib_marker, &update_msg->count_path_attr);
    ...
    if ((size - bytes_check) > 0) {
      bytes_read = libparsebgp_update_msg_parse_nlri_data_v4(
        nlri_ptr, (size - read_size), &update_msg->nlri,
        &update_msg->count_nlri);
 ...
 * \param [out]  attrs          Reference to the parsed attr map - will be
 * updated
 */
static void
                                          ...
                                          uint8_t **data)
{
  ...
  as_path_segment *as_segment =
    (as_path_segment *)malloc(sizeof(as_path_segment));

  if (path_len <
      4) // Nothing to parse if length doesn't include at least one asn
    return;

  /*
   * Per draft-ietf-grow-bmp, UPDATES must be sent as 4-octet, but this requires
   * the UPDATE to be modified. In draft 14 a new peer header flag indicates
   * size, but not all implementations support this draft yet.
   *
   *    IOS XE/XR does not modify the UPDATE and therefore a peers
   *    that is using 2-octet ASN's will not be parsed correctly.  Global
   * instance var four_octet_asn is used to check if the OPEN cap sent/recv
   * 4-octet or not. A compliant BMP implementation will still use 4-octet
   * even if the peer is 2-octet, so a check is needed to see if the as path
   * is encoded using 4 or 2 octet. This check is only done once.
   *
   *    This is temporary and can be removed after all implementations are
   * complete with bmp draft 14 or greater.
   */
  // TODO: talk to Alistair
  //    if (not update_msg->peer_inf->checked_asn_octet_length and not
  //    update_msg->four_octet_asn)
  //    {
  //        /*
  ...
  //    char asn_octet_size = (update_msg->peer_inf->using_2_octet_asn and not
  //    update_msg->four_octet_asn) ? 2 : 4;
  ...
  while (path_len > 0) {
    ...
    *data += 1;
    as_segment->seg_len = **data;
    *data += 1; // Count of AS's, not bytes
    path_len -= 2;

    // SELF_DEBUG("%s: rtr=%s: as_path seg_len = %d seg_type = %d, path_len = %d
    // total_len = %d as_octet_size = %d",
    //           peer_addr.c_str(), router_addr.c_str(),
    ...
    //
    //            //LOG_NOTICE("%s: rtr=%s: Could not parse the AS PATH due to
    //            update message buffer being too short when using ASN octet
    //            size %d",
    //            //           peer_addr.c_str(), router_addr.c_str(),
    //            asn_octet_size);
    ...
    as_segment->seg_asn =
      (uint32_t *)malloc(as_segment->seg_len * sizeof(uint32_t));
  ...
  // SELF_DEBUG("%s: rtr=%s: Parsed AS_PATH count %hu : %s", peer_addr.c_str(),
  // router_addr.c_str(), as_path_cnt, decoded_path.c_str());
  ...
  //        attrs[ATTR_TYPE_INTERNAL_AS_ORIGIN] = decoded_path.substr(spos,
  //        (epos - spos) + 1);
}
                                            ...
                                            uint8_t **data)
{
  ...
  if (path_attrs->attr_len == 8) { // RFC6793 ASN of 4 octets
    ...
    *data += 2;
    ...
    // LOG_ERR("%s: rtr=%s: path attribute is not the correct size of 6 or 8
    // octets.", peer_addr.c_str(), router_addr.c_str());  throw "path attribute
    // is not the correct size of 6 or 8 octets";
    return;
  }
}
  ...
  update_path_attrs *path_attrs, uint8_t *data, int *has_end_of_rib_marker)
{
  ...
  case ATTR_TYPE_ATOMIC_AGGREGATE: // Atomic aggregate
    //                parsed_data.attrs[ATTR_TYPE_ATOMIC_AGGREGATE] =
    //                std::string("1"); path_attrs->attr_value.origin = 1;
    break;
  ...
  {
    path_attrs->attr_value.attr_type_comm =
      (uint16_t *)malloc(path_attrs->attr_len / 2 * sizeof(uint16_t));
    ...
    for (int i = 0; i < path_attrs->attr_len; i += 2) {
      ...
      path_attrs->attr_value
        .attr_type_comm[path_attrs->count_attr_type_comm++] = value16bit;
    }
    break;
  }
  ...
  case ATTR_TYPE_IPV6_EXT_COMMUNITY: // IPv6 specific extended community list
                                     // (RFC 5701)
  {
  ...
  {
    libparsebgp_mp_reach_attr_parse_reach_nlri_attr(
      path_attrs, path_attrs->attr_len, &data);
    break;
  }

  case ATTR_TYPE_MP_UNREACH_NLRI: // RFC4760
  {
    libparsebgp_mp_un_reach_attr_parse_un_reach_nlri_attr(
      path_attrs, path_attrs->attr_len, &data, has_end_of_rib_marker);
    break;
  }
                                                ...
                                                uint16_t *count_path_attrs)
{
  ...
  *update_msg = (update_path_attrs **)malloc(sizeof(update_path_attrs *));
  update_path_attrs *path_attrs =
    (update_path_attrs *)malloc(sizeof(update_path_attrs));
  ...
  for (int read = 0; read < len; read += 2) {
    ...
    if (path_attrs->attr_len > 0 || (read + path_attrs->attr_len) <= len) {
       ...
       */
      libparsebgp_update_msg_parse_attr_data(path_attrs, data,
                                             has_end_of_rib_marker);
    ...
    (*update_msg)[*count_path_attrs] =
      (update_path_attrs *)malloc(sizeof(update_path_attrs));
    memcpy((*update_msg)[(*count_path_attrs)++], path_attrs,
           sizeof(update_path_attrs));
  }
  free(path_attrs);
  return read_size;
}
  ...
  update_path_attrs *path_attrs)
{
  ...
  case ATTR_TYPE_MP_UNREACH_NLRI: {
    free(path_attrs->attr_value.mp_unreach_nlri_data.withdrawn_routes_nlri
           .evpn_withdrawn);
    path_attrs->attr_value.mp_unreach_nlri_data.withdrawn_routes_nlri
      .evpn_withdrawn = NULL;
  ...
  case ATTR_TYPE_MP_REACH_NLRI: {
    ...
    for (int count = 0;
         count < path_attrs->attr_value.mp_reach_nlri_data.count_mp_rch_ls;
         ++count) {
      switch (path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
                .mp_rch_ls[count]
                .nlri_type) {
      case NLRI_TYPE_NODE: {
        free(path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
               .mp_rch_ls[count]
               .nlri_ls.node_nlri.local_nodes);
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
          .mp_rch_ls[count]
          .nlri_ls.node_nlri.local_nodes = NULL;
        break;
      }
      ...
      case NLRI_TYPE_LINK: {
        free(path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
               .mp_rch_ls[count]
               .nlri_ls.link_nlri.local_nodes);
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
          .mp_rch_ls[count]
          .nlri_ls.link_nlri.local_nodes = NULL;

        free(path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
               .mp_rch_ls[count]
               .nlri_ls.link_nlri.link_desc);
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
          .mp_rch_ls[count]
          .nlri_ls.link_nlri.link_desc = NULL;

        free(path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
               .mp_rch_ls[count]
               .nlri_ls.link_nlri.remote_nodes);
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
          .mp_rch_ls[count]
          .nlri_ls.link_nlri.remote_nodes = NULL;
      ...
      case NLRI_TYPE_IPV6_PREFIX: {
        free(path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
               .mp_rch_ls[count]
               .nlri_ls.prefix_nlri_ipv4_ipv6.local_nodes);
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
          .mp_rch_ls[count]
          .nlri_ls.prefix_nlri_ipv4_ipv6.local_nodes = NULL;

        free(path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
               .mp_rch_ls[count]
               .nlri_ls.prefix_nlri_ipv4_ipv6.prefix_desc);
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
          .mp_rch_ls[count]
          .nlri_ls.prefix_nlri_ipv4_ipv6.prefix_desc = NULL;
        break;
      }
      }
    }
