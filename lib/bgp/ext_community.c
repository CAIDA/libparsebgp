 ...
 * \param [in]   is_global_4bytes  True if the global admin field is 4 bytes,
 * false if 2 \param [in]   is_global_ipv4    True if the global admin field is
 * an IPv4 address, false if not
 *
                               ...
                               int is_global_4bytes, int is_global_ipv4)
{
  ...
  case EXT_COMMON_CISCO_VPN_ID:
    ...
    //                val_ss << "vpn-id=" << ipv4_char << ":0x" << std::hex <<
    //                val_16b;

    else if (is_global_4bytes)
      sprintf(ec_hdr->val, "vpn-id=%d:%d", val_32b, val_16b);
    //                val_ss << "vpn-id=" << val_32b << ":0x" << std::hex <<
    //                val_16b;

    else
      sprintf(ec_hdr->val, "vpn-id=%d:%d", val_16b, val_32b);
    //                val_ss << "vpn-id=" << val_16b << ":0x" << std::hex <<
    //                val_32b;
  ...
  default:
    // LOG_INFO("%s: Extended community common type %d subtype = %d is not yet
    // supported", peer_addr.c_str(),
                                ...
                                int is_global_4bytes, int is_global_ipv4)
{
  ...
  case EXT_GENERIC_OSPF_DOM_ID:     // deprecated
    // LOG_INFO("%s: Ignoring deprecated extended community %d/%d",
    // peer_addr.c_str(),
    //        ec_hdr.high_type, ec_hdr.low_type);
    break;
  ...
  case EXT_GENERIC_LAYER2_INFO: { // rfc4761
    ...
    default:
      sprintf(ec_hdr->val, "12info=%d:%c:mtu:%d", (int)encap_type, ctrl_flags,
              val_16b);
  ...
  case EXT_GENERIC_FLOWSPEC_TRAFFIC_RATE: {
    // 4 byte float
    // TODO: would prefer to use std::defaultfloat, but this is not available in
    // centos6.5 gcc
  ...
  update_path_attrs *path_attrs, u_char **data)
{
  ...
  if ((path_attrs->attr_len % 8)) {
    // LOG_NOTICE("%s: Parsing extended community len=%d is invalid, expecting
    // divisible by 8", peer_addr.c_str(), attr_len);
    return;
  }

  path_attrs->attr_value.ext_comm =
    (extcomm_hdr *)malloc(path_attrs->attr_len / 8 * sizeof(extcomm_hdr));
  uint16_t count = 0;
  /*
  ...
  for (int i = 0; i < path_attrs->attr_len; i += 8) {
    ...
    case EXT_TYPE_QOS_MARK:
      break; // TODO: Implement
    case EXT_TYPE_FLOW_SPEC:
      break; // TODO: Implement
    case EXT_TYPE_COS_CAP:
      break; // TODO: Implement
    default:
      break;
      // LOG_INFO("%s: Extended community type %d,%d is not yet supported",
      // peer_addr.c_str(),ec_hdr.high_type, ec_hdr.low_type);
    }
 ...
 *
 * \details
 *      Decodes the IPv6 specific and 2-octet, 4-octet.  This is pretty much the
 * as common for IPv4, but with some differences. Converts to human readable
 * form.
 *
 * \param [in]   ec_hdr          Reference to the extended community header
 *
...
static void decodeType_ipv6_specific(extcomm_hdr *ec_hdr, u_char **value)
{
  ...
  case EXT_IPV6_CISCO_VPN_ID:
    ...
    //            val_ss << "vpn-id=" << ipv6_char << ":0x" << std::hex <<
    //            val_16b;
  ...
  default:
    // LOG_INFO("%s: Extended community ipv6 specific type %d subtype = %d is
    // not yet supported", peer_addr.c_str(),
    //        ec_hdr.high_type, ec_hdr.low_type);
    break;
  }
  ...
  update_path_attrs *path_attrs, u_char **data)
{
  ...
  // LOG_INFO("%s: Parsing IPv6 extended community len=%d", peer_addr.c_str(),
  // attr_len);

  if ((path_attrs->attr_len % 20)) {
    // LOG_NOTICE("%s: Parsing IPv6 extended community len=%d is invalid,
    // expecting divisible by 20", peer_addr.c_str(), attr_len);
    return;
  }
