...
typedef struct open_capabilities {
  ...
  uint8_t count_add_path_capabilities; ///< Number of add_path_capability in
                                       ///< cap_values
} open_capabilities;
 ...
 * @param [in]   data                Pointer to raw bgp payload data, starting
 * at the notification message
 * @param [in]   size                Size of the data parsed_msg.error_textfer,
 * to prevent overrun when reading
 * @param [in]   openMessageIsSent   If open message is sent. False if received
 *
 * @return negative values indicate error, otherwise a positive value indicating
