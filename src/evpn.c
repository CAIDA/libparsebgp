 ...
 *
 * \details
 *      Will parse the Segment Identifier. Based on
 * https://tools.ietf.org/html/rfc7432#section-5
 *
 * \param [in/out]  data_pointer  Pointer to the beginning of Route
 * Distinguisher \param [out]     rd_type                    Reference to RD
 * type. \param [out]     rd_assigned_number         Reference to Assigned
  ...
  ethernet_segment_identifier *parsed_data)
{
  ...
  case 3: {
    ...
    *data_pointer += 6;
    ...
    parsed_data->type_3.local_discriminator_value =
      parsed_data->type_3.local_discriminator_value >> 8;
    break;
  }
  case 4: {
 ...
 *
 * \details
 *      Will parse the Route Distinguisher. Based on
...
// TODO: Refactor this method as it's overloaded - each case statement can be
// its own method
/**
 ...
 *
 * \details
 *      Parsing based on https://tools.ietf.org/html/rfc7432.  Will process all
 * NLRI's in data.
 *
 * \param [in]   data                   Pointer to the start of the prefixes to
 * be parsed \param [in]   data_len               Length of the data in bytes to
 * be read
 *
 */
                                         ...
                                         bool is_unreach)
{
  ...
  if (is_unreach)
    path_attrs->attr_value.mp_unreach_nlri_data.withdrawn_routes_nlri
      .evpn_withdrawn = (evpn_tuple *)malloc(sizeof(evpn_tuple));
  else
    path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info.evpn =
      (evpn_tuple *)malloc(sizeof(evpn_tuple));
  ...
  while ((data_read + 10 /* min read */) < data_len) {
    ...
    case EVPN_ROUTE_TYPE_ETHERNET_AUTO_DISCOVERY: {
      libparsebgp_evpn_parse_route_distinguisher(
        data_pointer, &tuple->route_type_specific.eth_ad_route.rd);
      *data_pointer += 8;
      ...
      if ((data_read + 17 /* expected read size */) <= data_len) {
        ...
        libparsebgp_evpn_parse_ethernet_segment_identifier(
          path_attrs, data_pointer,
          &tuple->route_type_specific.eth_ad_route.eth_seg_iden);
        *data_pointer += 10;

        // Ethernet Tag Id (4 bytes), printing in hex.

        memcpy(tuple->route_type_specific.eth_ad_route.ethernet_tag_id_hex,
               *data_pointer, 4);
        *data_pointer += 4;

        // MPLS Label (3 bytes)
        memcpy(&tuple->route_type_specific.eth_ad_route.mpls_label,
               *data_pointer, 3);
    ...
    case EVPN_ROUTE_TYPE_MAC_IP_ADVERTISMENT: {
      libparsebgp_evpn_parse_route_distinguisher(
        data_pointer, &tuple->route_type_specific.mac_ip_adv_route.rd);
      *data_pointer += 8;
      ...
      if ((data_read + 25 /* expected read size */) <= data_len) {
        ...
        libparsebgp_evpn_parse_ethernet_segment_identifier(
          path_attrs, data_pointer,
          &tuple->route_type_specific.mac_ip_adv_route.eth_seg_iden);
        *data_pointer += 10;
        ...
        *data_pointer += 4;
        ...
        memcpy(tuple->route_type_specific.mac_ip_adv_route.ethernet_tag_id_hex,
               *data_pointer, 4);
        ...
        addr_bytes =
          tuple->route_type_specific.mac_ip_adv_route.ip_addr_len > 0
            ? (tuple->route_type_specific.mac_ip_adv_route.ip_addr_len / 8)
            : 0;

        if (tuple->route_type_specific.mac_ip_adv_route.ip_addr_len > 0 &&
            (addr_bytes + data_read) <= data_len) {
        ...
        if ((data_read + 3) <= data_len) {
          ...
          // MPLS Label1 (3 bytes)
          memcpy(&tuple->route_type_specific.mac_ip_adv_route.mpls_label_1,
                 *data_pointer, 3);
        ...
        if (len == 3) {
          ...
          memcpy(&tuple->route_type_specific.mac_ip_adv_route.mpls_label_2,
                 *data_pointer, 3);
    ...
    case EVPN_ROUTE_TYPE_INCLUSIVE_MULTICAST_ETHERNET_TAG: {
      ...
      *data_pointer += 8;
      ...
      if ((data_read + 5 /* expected read size */) <= data_len) {
        ...
        // Ethernet Tag ID (4 bytes)
        memcpy(&tuple->route_type_specific.incl_multicast_eth_tag_route
                  .ethernet_tag_id_hex,
               *data_pointer, 4);
        ...
        addr_bytes =
          tuple->route_type_specific.incl_multicast_eth_tag_route.ip_addr_len >
              0
            ? (tuple->route_type_specific.incl_multicast_eth_tag_route
                 .ip_addr_len /
               8)
            : 0;

        if (tuple->route_type_specific.incl_multicast_eth_tag_route
                .ip_addr_len > 0 &&
            (addr_bytes + data_read) <= data_len) {

          // Originating Router's IP Address (4 or 16 bytes)
          memset(tuple->route_type_specific.incl_multicast_eth_tag_route
    ...
    case EVPN_ROUTE_TYPE_ETHERNET_SEGMENT_ROUTE: {
      libparsebgp_evpn_parse_route_distinguisher(
        data_pointer, &tuple->route_type_specific.eth_segment_route.rd);
      *data_pointer += 8;
      ...
      if ((data_read + 11 /* expected read size */) <= data_len) {
        ...
        libparsebgp_evpn_parse_ethernet_segment_identifier(
          path_attrs, data_pointer,
          &tuple->route_type_specific.eth_segment_route.eth_seg_iden);
        *data_pointer += 10;
        ...
        addr_bytes =
          tuple->route_type_specific.eth_segment_route.ip_addr_len > 0
            ? (tuple->route_type_specific.eth_segment_route.ip_addr_len / 8)
            : 0;

        if (tuple->route_type_specific.eth_segment_route.ip_addr_len > 0 &&
            (addr_bytes + data_read) <= data_len) {

          // Originating Router's IP Address (4 or 16 bytes)
          memset(
            tuple->route_type_specific.eth_segment_route.originating_router_ip,
            0, 16);
          memcpy(
            &tuple->route_type_specific.eth_segment_route.originating_router_ip,
            *data_pointer,
            (int)tuple->route_type_specific.eth_segment_route.ip_addr_len / 8);
          data_read += addr_bytes;
          len -= addr_bytes;
        }
      }
    ...
    if (is_unreach) {
      if (count_evpn_withdrawn)
        path_attrs->attr_value.mp_unreach_nlri_data.withdrawn_routes_nlri
          .evpn_withdrawn = (evpn_tuple *)realloc(
          path_attrs->attr_value.mp_unreach_nlri_data.withdrawn_routes_nlri
            .evpn_withdrawn,
          (count_evpn_withdrawn + 1) * sizeof(evpn_tuple));
      path_attrs->attr_value.mp_unreach_nlri_data.withdrawn_routes_nlri
        .evpn_withdrawn[count_evpn_withdrawn++] = *tuple;
    } else {
      if (count_evpn)
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info.evpn =
          (evpn_tuple *)realloc(
            path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info.evpn,
            (count_evpn + 1) * sizeof(evpn_tuple));
      path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info
        .evpn[count_evpn++] = *tuple;
    }
  }
