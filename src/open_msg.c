...
static ssize_t libparsebgp_open_msg_parse_capabilities(libparsebgp_open_msg_data *open_msg_data, u_char **data, size_t size, bool openMessageIsSent) {
    ...
    for (int i=0; i < size; ) {
        if(open_msg_data->count_opt_param - 1)
            open_msg_data->opt_param = (open_param *)realloc(open_msg_data->opt_param, (open_msg_data->count_opt_param)*sizeof(open_param));
                    ...
                    case BGP_CAP_ADD_PATH: {
                        ...
                        if (open_cap->cap_len >= 4) {
                            ...
                            for (int l = 0; l < open_cap->cap_len; l += 4) {
                                ...
                                open_cap->cap_values.add_path_data[open_cap->count_add_path_capabilities-1] = *add_path;
...
ssize_t libparsebgp_open_msg_parse_open_msg(libparsebgp_open_msg_data *open_msg_data, u_char *data, size_t size, bool openMessageIsSent) {
    ...
    if ( extract_from_buffer(&buf_ptr, &buf_size, &open_msg_data->asn, 2) != 2)
        return ERR_READING_MSG;
    if ( extract_from_buffer(&buf_ptr, &buf_size, &open_msg_data->hold_time, 2) != 2)
        return ERR_READING_MSG;
    if ( extract_from_buffer(&buf_ptr, &buf_size, &open_msg_data->bgp_id, 4) != 4)
        return ERR_READING_MSG;
    if ( extract_from_buffer(&buf_ptr, &buf_size, &open_msg_data->opt_param_len, 1) != 1)
        return ERR_READING_MSG;
    ...
    if (open_msg_data->opt_param_len == 0)
        ...
        libparsebgp_open_msg_parse_capabilities(open_msg_data,&buf_ptr, (size - read_size), openMessageIsSent);

        read_size += (size - read_size);

    } else {

        if (!libparsebgp_open_msg_parse_capabilities(open_msg_data,&buf_ptr, open_msg_data->opt_param_len, openMessageIsSent)) {
            return INVALID_MSG;
        }
