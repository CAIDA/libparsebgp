...
static ssize_t libparsebgp_parse_nlri_node(mp_reach_ls *mp_reach_ls, u_char **data, int data_len) {
    ...
    */
    mp_reach_ls->nlri_ls.node_nlri.local_nodes = (node_descriptor *)malloc(sizeof(node_descriptor));
    node_descriptor *info = (node_descriptor *)malloc(sizeof(node_descriptor));

    while (len > 0) {
        if(count)
            mp_reach_ls->nlri_ls.node_nlri.local_nodes = (node_descriptor *)realloc(mp_reach_ls->nlri_ls.node_nlri.local_nodes ,(count+1)*sizeof(node_descriptor));
    ...
    *data += 4; data_read += 4;
    ...
    switch (info->type) {
        case LINK_DESCR_ID:
        {
            ...
            memcpy(&info->local_id, *data, 4); SWAP_BYTES(&info->local_id, 4);
            memcpy(&info->remote_id, *data+4, 4); SWAP_BYTES(&info->remote_id, 4);
            data_read += 8;
        ...
        case LINK_DESCR_MT_ID:
        {
            ...
            memcpy(&info->mt_id, *data, info->len); SWAP_BYTES(&info->mt_id, info->len);
...
static void libparsebgp_parse_nlri_link(mp_reach_ls *mp_reach_ls, u_char **data, int data_len) {
    ...
    *data += 4;
    ...
    mp_reach_ls->nlri_ls.link_nlri.local_nodes = (node_descriptor *)malloc(sizeof(node_descriptor));
    node_descriptor *info = (node_descriptor *)malloc(sizeof(node_descriptor));
    ...
    *data += 4;
    ...
    mp_reach_ls->nlri_ls.link_nlri.remote_nodes = (node_descriptor *)malloc(sizeof(node_descriptor));
     ...
     */
    mp_reach_ls->nlri_ls.link_nlri.link_desc = (link_descriptor *)malloc(sizeof(link_descriptor));
    count = 0;
    link_descriptor *link_info = (link_descriptor *)malloc(sizeof(link_descriptor));
    while (data_len > 0) {
        if(count)
            mp_reach_ls->nlri_ls.link_nlri.link_desc = (link_descriptor *)realloc(mp_reach_ls->nlri_ls.link_nlri.link_desc ,(count+1)*sizeof(link_descriptor));
...
static void libparsebgp_parse_nlri_prefix(mp_reach_ls *mp_reach_ls, u_char **data, int data_len, bool isIPv4) {
    ...
    *data += 4;
    ...
    while (mp_reach_ls->nlri_ls.prefix_nlri_ipv4_ipv6.len > 0) {
        ...
        if(count)
            mp_reach_ls->nlri_ls.prefix_nlri_ipv4_ipv6.local_nodes = (node_descriptor *)realloc(mp_reach_ls->nlri_ls.prefix_nlri_ipv4_ipv6.local_nodes,(count+1)*sizeof(node_descriptor));
        ...
        mp_reach_ls->nlri_ls.prefix_nlri_ipv4_ipv6.local_nodes[count++]=*local_node;
        // Update the nlri data pointer and remaining length after processing the local descriptor sub-tlv
        *data += data_read;
        data_len -= data_read;
    }
     ...
     */
    count = 0;
    mp_reach_ls->nlri_ls.prefix_nlri_ipv4_ipv6.prefix_desc = (prefix_descriptor *)malloc(sizeof(prefix_descriptor));
    prefix_descriptor *info = (prefix_descriptor *)malloc(sizeof(prefix_descriptor));
    while (data_len > 0) {
        if(count)
            mp_reach_ls->nlri_ls.prefix_nlri_ipv4_ipv6.prefix_desc = (prefix_descriptor *)realloc(mp_reach_ls->nlri_ls.prefix_nlri_ipv4_ipv6.prefix_desc, (count+1)*sizeof(prefix_descriptor));
        memset(info, 0, sizeof(info));
...
static ssize_t libparsebgp_parse_link_state_nlri_data(update_path_attrs *path_attrs, u_char **data, uint16_t len) {
    ...
    while (nlri_len_read < len) {
        if(path_attrs->attr_value.mp_reach_nlri_data.count_mp_rch_ls)
            path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info.mp_rch_ls = (mp_reach_ls *)realloc(path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info.mp_rch_ls, (path_attrs->attr_value.mp_reach_nlri_data.count_mp_rch_ls+1)*sizeof(mp_reach_ls));
...
//                LOG_NOTICE("%s: bgp-ls: failed to parse link state NLRI; length is larger than available data",peer_addr.c_str());
        ...
        path_attrs->attr_value.mp_reach_nlri_data.mp_reach_nlri_info.mp_rch_ls[path_attrs->attr_value.mp_reach_nlri_data.count_mp_rch_ls++]=*mp_nlri_ls;
