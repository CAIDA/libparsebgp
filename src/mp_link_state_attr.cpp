...
//namespace bgp_msg {
    ...
    void libparsebgp_mp_link_state_attr_parse_attr_link_state(update_path_attrs *path_attrs, int attr_len, u_char *data) {
        ...
        int tlv_len;
        while (attr_len > 0) {
            tlv_len = libparsebgp_mp_link_state_attr_parse_attr_link_state_tlv(path_attrs, attr_len, data);
            attr_len -= tlv_len;
  ...
  //          LOG_NOTICE("%s: bgp-ls: failed to parse attribute; too short",peer_addr.c_str());
       ...
       //             LOG_INFO("%s: bgp-ls: node flag attribute length is too long %d should be 1",peer_addr.c_str(), len);
                ...
                std::string flags = parse_flags_to_string(*data, LS_FLAGS_NODE_NLRI, sizeof(LS_FLAGS_NODE_NLRI));

        //        SELF_DEBUG("%s: bgp-ls: parsed node flags %s %x (len=%d)", peer_addr.c_str(), flags.c_str(), *data, len);

                path_attrs->attr_value.ls_attrs[ATTR_NODE_FLAG].fill(0);
                strncpy((char *)path_attrs->attr_value.ls_attrs[ATTR_NODE_FLAG].data(), flags.c_str(), flags.size());
            }
            break;
    ...
    //                LOG_NOTICE("%s: bgp-ls: failed to parse attribute local router id IPv4 sub-tlv; too short",peer_addr.c_str());
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_NODE_IPV4_ROUTER_ID_LOCAL].data(), data, 4);
                inet_ntop(AF_INET, path_attrs->attr_value.ls_attrs[ATTR_NODE_IPV4_ROUTER_ID_LOCAL].data(), ip_char, sizeof(ip_char));
    ...
    //            SELF_DEBUG("%s: bgp-ls: parsed local IPv4 router id attribute: addr = %s", peer_addr.c_str(), ip_char);
     ...
     //               LOG_NOTICE("%s: bgp-ls: failed to parse attribute local router id IPv6 sub-tlv; too short",peer_addr.c_str());
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_NODE_IPV6_ROUTER_ID_LOCAL].data(), data, 16);
                inet_ntop(AF_INET6, path_attrs->attr_value.ls_attrs[ATTR_NODE_IPV6_ROUTER_ID_LOCAL].data(), ip_char, sizeof(ip_char));
      ...
      //          SELF_DEBUG("%s: bgp-ls: parsed local IPv6 router id attribute: addr = %s", peer_addr.c_str(), ip_char);
            ...
            case ATTR_NODE_ISIS_AREA_ID:
                if (len <= 8)
                    memcpy(path_attrs->attr_value.ls_attrs[ATTR_NODE_ISIS_AREA_ID].data(), data, len);
                    path_attrs->attr_value.ls_attrs[ATTR_NODE_ISIS_AREA_ID].data()[8] = len;
    ...
    //            SELF_DEBUG("%s: bgp-ls: parsing node MT ID attribute (len=%d)", peer_addr.c_str(), len);
                ...
                strncpy((char *)path_attrs->attr_value.ls_attrs[ATTR_NODE_MT_ID].data(), val_ss.str().data(), val_ss.str().length()+1);
                // LOG_INFO("%s: bgp-ls: parsed node MT_ID %s (len=%d)", peer_addr.c_str(), val_ss.str().c_str(), len);
                break;

            case ATTR_NODE_NAME:
                path_attrs->attr_value.ls_attrs[ATTR_NODE_NAME].fill(0);
                strncpy((char *)path_attrs->attr_value.ls_attrs[ATTR_NODE_NAME].data(), (char *)data, len);
        ...
        //        SELF_DEBUG("%s: bgp-ls: parsed node name attribute: name = %s", peer_addr.c_str(),parsed_data->ls_attrs[ATTR_NODE_NAME].data());
           ...
           //     LOG_INFO("%s: bgp-ls: opaque node attribute (len=%d), not yet implemented", peer_addr.c_str(), len);
            ...
            case ATTR_NODE_SR_CAPABILITIES: {
                ...
                // Decode flags
                if (strcmp(path_attrs->attr_value.ls.nodes.front().protocol, "IS-IS") >= 0) {
                    val_ss << parse_flags_to_string(*data, LS_FLAGS_SR_CAP_ISIS, sizeof(LS_FLAGS_SR_CAP_ISIS));

                } else if (strcmp(path_attrs->attr_value.ls.nodes.front().protocol, "OSPF") >= 0) {
        ...
        //        SELF_DEBUG("%s: bgp-ls: parsed node sr capabilities (len=%d) %s", peer_addr.c_str(), len, val_ss.str().c_str());
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_NODE_SR_CAPABILITIES].data(), val_ss.str().data(), val_ss.str().length());
                break;
            }
           ...
           //         LOG_NOTICE("%s: bgp-ls: failed to parse attribute link admin group sub-tlv, size not 4",peer_addr.c_str());
                    ...
                    memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_ADMIN_GROUP].data(), &value_32bit, 4);
            ...
            case ATTR_LINK_IGP_METRIC:
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_IGP_METRIC].data(), &value_32bit, 4);
      ...
      //          SELF_DEBUG("%s: bgp-ls: parsed link IGP metric attribute: metric = %u", peer_addr.c_str(), value_32bit);
            ...
            case ATTR_LINK_IPV4_ROUTER_ID_REMOTE:
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_IPV4_ROUTER_ID_REMOTE].data(), data, 4);
                inet_ntop(AF_INET, path_attrs->attr_value.ls_attrs[ATTR_LINK_IPV4_ROUTER_ID_REMOTE].data(), ip_char, sizeof(ip_char));
            ...
            case ATTR_LINK_IPV6_ROUTER_ID_REMOTE:
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_IPV6_ROUTER_ID_REMOTE].data(), data, 16);
                inet_ntop(AF_INET6, path_attrs->attr_value.ls_attrs[ATTR_LINK_IPV6_ROUTER_ID_REMOTE].data(), ip_char, sizeof(ip_char));
            ...
            case ATTR_LINK_MAX_LINK_BW:
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_MAX_LINK_BW].data(), &float_val, 4);
            ...
            case ATTR_LINK_MAX_RESV_BW:
                ...
                SWAP_BYTES(&float_val, len);
                float_val = ieee_float_to_kbps(float_val);
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_MAX_RESV_BW].data(), &float_val, 4);
            ...
            case ATTR_LINK_NAME: {
                path_attrs->attr_value.ls_attrs[ATTR_LINK_NAME].fill(0);
                strncpy((char *)path_attrs->attr_value.ls_attrs[ATTR_LINK_NAME].data(), (char *)data, len);
            ...
            case ATTR_LINK_ADJACENCY_SID: {
                ...
                if (strlen((char *)path_attrs->attr_value.ls_attrs[ATTR_LINK_ADJACENCY_SID].data()) > 0)
                    val_ss << ", ";

                // Decode flags
                if (strcmp(path_attrs->attr_value.ls.links.front().protocol, "IS-IS") >= 0) {
                    val_ss << parse_flags_to_string(*data,
                                                          LS_FLAGS_PEER_ADJ_SID_ISIS, sizeof(LS_FLAGS_PEER_ADJ_SID_ISIS));

                } else if (strcmp(path_attrs->attr_value.ls.links.front().protocol, "OSPF") >= 0) {
                    val_ss << parse_flags_to_string(*data,
                ...
                strncat((char *)path_attrs->attr_value.ls_attrs[ATTR_LINK_ADJACENCY_SID].data(),
                        val_ss.str().c_str(),
                        path_attrs->attr_value.ls_attrs[ATTR_LINK_ADJACENCY_SID].size() -
                                strlen((char *)path_attrs->attr_value.ls_attrs[ATTR_LINK_ADJACENCY_SID].data()));
                break;
            }
            ...
            case ATTR_LINK_TE_DEF_METRIC:
                ...
                if (len == 0) {
                    memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_TE_DEF_METRIC].data(), &value_32bit, len);
                    break;
                } else if (len > 4) {
            ...
            case ATTR_LINK_UNRESV_BW: {
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_UNRESV_BW].data(), val_ss.str().data(), val_ss.str().length());
            ...
            case ATTR_LINK_PEER_EPE_NODE_SID:
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_LINK_PEER_EPE_NODE_SID].data(), val_ss.str().data(), val_ss.str().length());
                break;
            ...
            case ATTR_PREFIX_PREFIX_METRIC:
                ...
                memcpy(path_attrs->attr_value.ls_attrs[ATTR_PREFIX_PREFIX_METRIC].data(), &value_32bit, 4);
            ...
            case ATTR_PREFIX_ROUTE_TAG:
            {
                ...
                if (len == 4) {
                    ...
                    memcpy(path_attrs->attr_value.ls_attrs[ATTR_PREFIX_ROUTE_TAG].data(), &value_32bit, 4);
...
//                             value_32bit, len);
            ...
            case ATTR_PREFIX_SID: {
                ...
                if (strlen((char *)path_attrs->attr_value.ls_attrs[ATTR_PREFIX_SID].data()) > 0)
                    val_ss << ", ";
                ...
                // Decode flags
                if (strcmp(path_attrs->attr_value.ls.prefixes.front().protocol, "IS-IS") >= 0) {
                    val_ss << parse_flags_to_string(*data,
                                                    LS_FLAGS_PREFIX_SID_ISIS, sizeof(LS_FLAGS_PREFIX_SID_ISIS));

                } else if (strcmp(path_attrs->attr_value.ls.prefixes.front().protocol, "OSPF") >= 0) {
                    val_ss << parse_flags_to_string(*data,
                ...
                strncat((char *)path_attrs->attr_value.ls_attrs[ATTR_PREFIX_SID].data(),
                        val_ss.str().c_str(),
                        path_attrs->attr_value.ls_attrs[ATTR_PREFIX_SID].size() -
                                strlen((char *)path_attrs->attr_value.ls_attrs[ATTR_PREFIX_SID].data()));
