...
bool parseMRT::parseMsg(u_char *&buffer, int& buf_len)
{
    ...
    try {
        mrt_type = parse_common_header(buffer, buf_len);
        ...
        switch (mrt_type) {
            ...
            case MRT_TYPE::TABLE_DUMP : {
                buffer_mrt_message(buffer, buf_len);
                parse_table_dump(mrt_data, mrt_data_len);
                break;
            }

            case MRT_TYPE::TABLE_DUMP_V2 : {
                buffer_mrt_message(buffer, buf_len);
                parse_table_dump_v2(mrt_data, mrt_data_len);
                break;
            }
            ...
            case MRT_TYPE::BGP4MP_ET : {
                buffer_mrt_message(buffer, buf_len);
                parse_bgp4mp(mrt_data, mrt_data_len);
                break;
            }
...
void parseMRT::parse_table_dump(u_char *buffer, int& buf_len)
{
    string peer_info_key;
    u_char local_addr[16];
    if (extract_from_buffer(buffer, buf_len, &table_dump.view_number, 2) != 2)
        throw "Error in parsing view number";
    if (extract_from_buffer(buffer, buf_len, &table_dump.sequence, 2) != 2)
    ...
    if (extract_from_buffer(buffer, buf_len, &table_dump.prefix_len, 1) != 1)
        throw "Error in parsing prefix length";

    if (extract_from_buffer(buffer, buf_len, &table_dump.status, 1) != 1)
        throw "Error in parsing status";

    if (extract_from_buffer(buffer, buf_len, &table_dump.originated_time, 4) != 4)
    ...
    if (extract_from_buffer(buffer, buf_len, &table_dump.peer_as, 2) != 2)
        throw "Error in parsing peer AS";

    if (extract_from_buffer(buffer, buf_len, &table_dump.attribute_len, 2) != 2)
        throw "Error in parsing attribute length";

    if (extract_from_buffer(buffer, buf_len, &table_dump.bgp_attribute, table_dump.attribute_len) != table_dump.attribute_len)
    ...
    uMsg->parseAttributes(table_dump.bgp_attribute, table_dump.attribute_len, bgp_msg.parsed_data, bgp_msg.hasEndOfRIBMarker);
...
void parseMRT::parse_table_dump_v2(u_char *buffer, int& buf_len) {
    switch (c_hdr.sub_type) {
        case PEER_INDEX_TABLE:
            parse_peer_index_table(buffer,buf_len);
            break;

        case RIB_IPV4_UNICAST:
            parse_rib_unicast(buffer,buf_len);
            break;
        ...
        case RIB_GENERIC:
            parse_rib_generic(buffer,buf_len);
            break;
    }
}

void parseMRT::parse_peer_index_table(unsigned char *buffer, int& buf_len)
{
    uint16_t count = 0;
    uint8_t  AS_num;
    uint8_t  Addr_fam;
    ...
    bgp::SWAP_BYTES(&peer_index_table.peer_count);
...
void parseMRT::parse_rib_unicast(unsigned char *buffer, int& buf_len)
{
    uint16_t count = 0;
    string peer_info_key;

    if (extract_from_buffer(buffer, buf_len, &rib_entry_header.sequence_number, 4) != 4)
        throw "Error in parsing sequence number";

    bgp::SWAP_BYTES(&rib_entry_header.sequence_number);

    if (extract_from_buffer(buffer, buf_len, &rib_entry_header.prefix_length, 1) != 1)
        throw "Error in parsing view_name_length";

    u_char local_addr[rib_entry_header.prefix_length/8];

    if (extract_from_buffer(buffer, buf_len, &local_addr, rib_entry_header.prefix_length/8) != rib_entry_header.prefix_length/8)
        throw "Error in parsing prefix";

    switch (c_hdr.sub_type) {
        case RIB_IPV4_UNICAST:
            inet_ntop(AF_INET, local_addr, rib_entry_header.prefix, sizeof(rib_entry_header.prefix));
            break;
        case RIB_IPV6_UNICAST:
            inet_ntop(AF_INET6, local_addr, rib_entry_header.prefix, sizeof(rib_entry_header.prefix));
            break;
    }
    if (extract_from_buffer(buffer, buf_len, &rib_entry_header.entry_count, 2) != 2)
        throw "Error in parsing peer count";

    bgp::SWAP_BYTES(&rib_entry_header.entry_count);

    while (count < rib_entry_header.entry_count) {
...
//            throw "Error in parsing bgp_attribute";
        ...
        rib_entry_header.rib_entries.push_back(*r_entry);
        delete uMsg;
        delete r_entry;
        count++;
    }
...
void parseMRT::parse_rib_generic(unsigned char *buffer, int& buf_len)
{
    uint16_t count = 0;
    uint8_t  IPlen;
    u_char* local_addr;
    ...
    if (extract_from_buffer(buffer, buf_len, &rib_generic_entry_header.sequence_number, 4) != 4)
        throw "Error in parsing sequence number";

    if (extract_from_buffer(buffer, buf_len, &rib_generic_entry_header.address_family_identifier, 2) != 2)
        throw "Error in parsing address_family_identifier";

    if (extract_from_buffer(buffer, buf_len, &rib_generic_entry_header.subsequent_afi, 1) != 1)
...
void parseMRT::parse_bgp4mp(unsigned char* buffer, int& buf_len) {
    string peer_info_key;
    /*switch (c_hdr.subType) {
        ...
        case BGP4MP_MESSAGE: {
            parseBGP4MPaux(&bgp4mp_msg, buffer, buf_len, false, false);
            peer_info_key =  bgp4mp_msg.peer_IP;
...
//            pBGP = new parseBGP(bgp4mp_msg_as4.peer_IP, bgp4mp_msg_as4.peer_AS_number, (bgp4mp_msg_as4.address_family == AFI_IPv4), c_hdr.timeStamp, c_hdr.microsecond_timestamp, &peer_info_map[peer_info_key]);
            parseBGP4MPaux(&bgp4mp_msg, buffer, buf_len, true, false);
            peer_info_key =  bgp4mp_msg.peer_IP;
            pBGP = new parseBGP(bgp4mp_msg.peer_IP, bgp4mp_msg.peer_AS_number, (bgp4mp_msg.address_family == AFI_IPv4), c_hdr.timeStamp, c_hdr.microsecond_timestamp, &peer_info_map[peer_info_key]);
            break;
        }
...
//            pBGP = new parseBGP(bgp4mp_msg_as4_local.peer_IP, bgp4mp_msg_as4_local.peer_AS_number, (bgp4mp_msg_as4_local.address_family == AFI_IPv4), c_hdr.timeStamp, c_hdr.microsecond_timestamp, &peer_info_map[peer_info_key]);
    ...
    switch (c_hdr.sub_type) {
        case BGP4MP_STATE_CHANGE:
        case BGP4MP_STATE_CHANGE_AS4: {
            int asn_len = (c_hdr.sub_type == BGP4MP_STATE_CHANGE_AS4) ? 8 : 4;
            int ip_addr_len = 4;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_state_change, asn_len) != asn_len)  // ASNs
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_state_change.interface_index, 2) != 2)
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_state_change.address_family, 2) != 2)
                throw;
            if (bgp4mp_state_change.address_family == AFI_IPv6)
                ip_addr_len = 16;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_state_change.peer_ip, ip_addr_len) != ip_addr_len)
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_state_change.local_ip, ip_addr_len) != ip_addr_len)
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_state_change.old_state, 2) != 2)
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_state_change.new_state, 2) != 2)
                throw;
            break;
        }
        case BGP4MP_MESSAGE:
        ...
        case BGP4MP_MESSAGE_LOCAL:
        case BGP4MP_MESSAGE_AS4_LOCAL: {
            int asn_len = (c_hdr.sub_type == BGP4MP_MESSAGE_AS4 || c_hdr.sub_type == BGP4MP_MESSAGE_AS4_LOCAL) ? 8 : 4;
            int ip_addr_len = 4;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_msg, asn_len) != asn_len) //ASNs
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_msg.interface_index, 2) != 2)
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_msg.address_family, 2) != 2)
                throw;
            bgp::SWAP_BYTES(&bgp4mp_msg.peer_asn);
            bgp::SWAP_BYTES(&bgp4mp_msg.local_asn);
            bgp::SWAP_BYTES(&bgp4mp_msg.interface_index);
            bgp::SWAP_BYTES(&bgp4mp_msg.address_family);
            if (bgp4mp_msg.address_family == AFI_IPv6)
                ip_addr_len = 16;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_msg.peer_ip, ip_addr_len) != ip_addr_len)
                throw;
            if (extract_from_buffer(buffer, buf_len, &bgp4mp_msg.local_ip, ip_addr_len) != ip_addr_len)
                throw;
            ...
            if (mrt_data_len != buf_len)
                throw;
            memcpy(&bgp4mp_msg.bgp_data, buffer, mrt_data_len);
            ...
            p_entry.peer_as = bgp4mp_msg.peer_asn;
            p_entry.timestamp_secs = c_hdr.time_stamp;
            p_entry.timestamp_us = c_hdr.microsecond_timestamp;

            pbgp = new parseBGP(&p_entry, "", &peer_info_map[peer_info_key]);
            uint32_t asn = (c_hdr.sub_type > 5) ? bgp4mp_msg.local_asn : bgp4mp_msg.peer_asn;
            if (pbgp->parseBGPfromMRT(buffer, mrt_data_len, &bgp_msg, &up_event, &down_event, asn, c_hdr.sub_type > 5) == parseBGP::BGP_MSG_OPEN) {
                up_event.local_asn = bgp4mp_msg.local_asn;
                up_event.remote_asn = bgp4mp_msg.peer_asn;
 ...
 */
uint32_t parseMRT::get_mrt_length() {
    return mrt_len;
}
 ...
 * \throws String error
 */
void parseMRT::buffer_mrt_message(u_char *& buffer, int& buf_len) {
    if (mrt_len <= 0)
        return;
